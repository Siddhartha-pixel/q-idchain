...
export default function Groups() {
  const { identity, contacts, groups, groupMessages, addGroup, addGroupMsg } = useStore()
  ...
  useEffect(() => {
    if (!identity?.did || groups.length === 0) return
  ...
  useEffect(() => {
    ...
    const handler = payload => {
      try {
        ...
        const existing = (state.groupMessages[payload.groupId] || []).some(m => m.id === payload.id)
        if (existing) return

        const plain = symmetricDecrypt(
          { ciphertext: payload.encrypted, nonce: payload.nonce },
          group.sessionKey
        )

        addGroupMsg(payload.groupId, {
          ...payload,
          plaintext: plain,
          read: payload.from === state.identity?.did,
        })
      } catch (e) {
        console.error('Group decrypt failed:', e.message)
      }
    }

    sock.on('group:message', handler)
    return () => {
      sock.off('group:message', handler)
    }
  }, [addGroupMsg])

  function createGroup() {
    if (!gName.trim() || !identity?.did) return
    ...
    const group = {
      ...
      members: [
        ...
        ...gMembers
          ...
          .map(did => ({
            did,
            alias: contacts.find(c => c.did === did)?.alias || did.slice(0, 12),
            role: 'member',
          })),
      ],
      createdAt: Date.now(),
  ...
  function sendGroupMsg() {
    if (!input.trim() || !selected || !identity?.did) return

    const text = input.trim()
    const enc = symmetricEncrypt(text, selected.sessionKey)
  ...
  function memberAlias(did) {
    return did === identity?.did
      ? 'You'
      : (selected?.members.find(m => m.did === did)?.alias || did.slice(0, 10))
  }
      ...
      <div style={{ width: 240, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', background: 'var(--bg-deep)', flexShrink: 0 }}>
        <div style={{ padding: '12px 14px 10px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            <button onClick={() => setShowCreate(v => !v)} style={{ background: 'var(--purple-glow)', border: '1px solid rgba(157,78,221,.35)', borderRadius: 6, color: 'var(--purple)', cursor: 'pointer', padding: '3px 9px', fontSize: '0.72rem', fontFamily: 'var(--font-mono)' }}>
              + NEW
            </button>
          </div>
          ...
          {showCreate && (
            <div>
              ...
              <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginBottom: 5, fontFamily: 'var(--font-mono)' }}>
                ADD MEMBERS:
              </div>
              ...
              {contacts.map(c => (
                <label key={c.did} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, cursor: 'pointer', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                  <input
                    type="checkbox"
                    checked={gMembers.includes(c.did)}
                    onChange={e =>
                      setGMembers(prev =>
                        e.target.checked
                          ? [...new Set([...prev, c.did])]
                          : prev.filter(d => d !== c.did)
                      )
                    }
                  />
                  {c.alias}
              ...
              <button
                ...
                style={{ width: '100%', justifyContent: 'center', padding: '7px', fontSize: '0.75rem', marginTop: 6 }}
                onClick={createGroup}
              >
                CREATE GROUP
              </button>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {groups.length === 0 ? (
            <div style={{ padding: 18, textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.8rem' }}>
              No groups yet.
            </div>
          ) : groups.map(g => (
            <div
              ...
              style={{
                ...
                cursor: 'pointer',
                background: selected?.id === g.id ? 'var(--purple-glow)' : 'transparent',
                transition: 'background 0.13s',
              }}
            >
              <div style={{ fontWeight: 500, fontSize: '0.88rem', color: 'var(--text-primary)', marginBottom: 2 }}>
                {g.name}
              </div>
              <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)' }}>
                {g.members.length} members · v{g.keyVersion}
              </div>
            </div>
          ))}
        </div>
      ...
      {selected ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-surface)' }}>
          <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-deep)', flexShrink: 0 }}>
            <div>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '0.95rem', letterSpacing: '0.05em' }}>
                {selected.name}
              </div>
            ...
            {msgs.length === 0 ? (
              ...
              return (
                <div key={m.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isOwn ? 'flex-end' : 'flex-start' }}>
                  {!isOwn && (
                    <div style={{ fontSize: '0.65rem', color: 'var(--purple)', fontFamily: 'var(--font-mono)', marginBottom: 2 }}>
                      {memberAlias(m.from)}
                    </div>
                  )}
                  ...
                  <div
                    style={{
                      ...
                      background: isOwn ? 'rgba(157,78,221,.1)' : 'var(--bg-card)',
                      border: `1px solid ${isOwn ? 'rgba(157,78,221,.3)' : 'var(--border)'}`,
          ...
          <div style={{ padding: '12px 18px', borderTop: '1px solid var(--border)', display: 'flex', gap: 9, background: 'var(--bg-deep)', flexShrink: 0 }}>
            <input
              className="cyber-input"
              placeholder={`Message ${selected.name} (group encrypted)`}
              value={input}
            ...
            <button
              className="cyber-btn"
              style={{ borderColor: 'var(--purple)', color: 'var(--purple)', flexShrink: 0 }}
