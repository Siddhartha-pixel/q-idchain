...
export default function Dashboard() {
  ...
  const { identity, contacts, conversations, groups, sharedFiles } = useStore()
  ...
  const totalUnread = contacts.reduce(
    (s, c) => s + (conversations[c.did]?.filter(m => !m.read && m.from !== identity?.did).length || 0),
    0
  )

  const totpEnabled = !!useStore.getState().totpSecret
    ...
    <div className="animate-fade">
      <div style={{ marginBottom: 26 }}>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.88rem' }}>
          Welcome back, <strong style={{ color: 'var(--cyan)' }}>{identity?.alias}</strong> ·{' '}
          {isConnected() ? <span style={{ color: 'var(--green)' }}>● Connected</span> : <span style={{ color: 'var(--amber)' }}>○ Reconnecting...</span>}
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 14, marginBottom: 26 }}>
        {stats.map(s => (
          <div key={s.label} className="cyber-card" style={{ padding: '18px 14px', textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.9rem', fontWeight: 700, color: s.color }}>
              {s.value}
            </div>
            <div style={{ fontSize: '0.62rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', letterSpacing: '0.1em', marginTop: 3 }}>
              {s.label}
            </div>
          </div>
        ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 18 }}>
        <div className="cyber-card" style={{ padding: 22 }}>
          <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', letterSpacing: '0.1em', marginBottom: 14 }}>
            YOUR DECENTRALIZED IDENTITY
          </div>

          <div style={{ marginBottom: 10 }}>
            <div style={{ fontSize: '0.62rem', color: 'var(--text-muted)', marginBottom: 3 }}>DID</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', color: 'var(--cyan)', wordBreak: 'break-all', lineHeight: 1.5 }}>
              {truncateDID(identity?.did, 22)}
            </div>
          </div>

          {[
            ['ENCRYPTION', 'X25519 + Kyber-768 (Hybrid PQC)', 'var(--green)'],
            ['SIGNING', 'Ed25519', 'var(--purple)'],
            ['2FA', 'TOTP — Google Authenticator', 'var(--cyan)'],
            ['STORAGE', 'IPFS Decentralized', 'var(--amber)'],
          ].map(([l, v, c]) => (
        ...
        <div className="cyber-card" style={{ padding: 22 }}>
          ...
          {[
            ...
            ['TOTP 2FA (Google Authenticator)', totpEnabled],
            ...
            <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: '1px solid rgba(0,229,255,.05)' }}>
              ...
              <span style={{ fontSize: '0.65rem', fontFamily: 'var(--font-mono)', color: ok ? 'var(--green)' : 'var(--amber)' }}>
                {ok ? '✓ ON' : '⚠ SETUP'}
              </span>
            </div>
          ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
        ...
        <div className="cyber-card" style={{ padding: 22 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
            <button className="cyber-btn primary" style={{ justifyContent: 'center' }} onClick={() => navigate('/app/messages')}>
              ◈ New Message
            </button>
            <button className="cyber-btn" style={{ justifyContent: 'center' }} onClick={() => navigate('/app/files')}>
              ◰ Share File
            </button>
            <button className="cyber-btn" style={{ justifyContent: 'center' }} onClick={() => navigate('/app/wallet')}>
              ◉ View Identity
            </button>
          </div>
        </div>
