...
import { createIdentity, setupTOTP, verifyTOTP, generateBackupCodes, truncateDID } from '../lib/identity.js'
...
export default function Auth() {
  ...
  async function doRegister() {
    ...
    if (step === 0) {
      ...
      try {
        ...
        setNewId(id); setTotp(t); setCodes(c); setStep(1)
      } catch(e) { setError(e.message) }
      finally { setLoading(false) }
    } else if (step === 1) {
      if (!await verifyTOTP(totpToken, totp.secret)) return setError('Invalid code — open Google Authenticator, enter the current 6-digit code for Q-IDChain')
      setStep(2)
    } else if (step === 2) {
      setStep(3)
    } else {
  ...
  async function doLogin() {
    ...
    if (storedTOTP) {
      if (!await verifyTOTP(loginOTP, storedTOTP)) return setError('Invalid Google Authenticator code')
    }
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg-void)', padding:24 }}>
      ...
      <div className="cyber-card" style={{ width:'100%', maxWidth:440, padding:30, position:'relative', zIndex:5 }}>
        ...
        <div style={{ textAlign:'center', marginBottom:26 }}>
          ...
          <div style={{ display:'flex', borderRadius:8, border:'1px solid var(--border)', overflow:'hidden' }}>
            {['register','login'].map(m => (
              <button key={m} onClick={() => { setMode(m); setStep(0); setError('') }} style={{ flex:1, padding:'9px', background: mode===m ? 'var(--cyan-glow)' : 'transparent', border:'none', color: mode===m ? 'var(--cyan)' : 'var(--text-secondary)', fontFamily:'var(--font-display)', fontWeight:600, letterSpacing:'0.07em', fontSize:'0.82rem', cursor:'pointer', transition:'all 0.14s' }}>
        ...
        {error && <div style={{ padding:'9px 13px', borderRadius:8, marginBottom:14, background:'var(--red-glow)', border:'1px solid rgba(255,59,92,.3)', color:'var(--red)', fontSize:'0.83rem' }}>⚠ {error}</div>}
        ...
        {mode === 'register' && <>
          ...
          <div style={{ display:'flex', gap:5, marginBottom:22 }}>
            {steps.map((l,i) => (
              <div key={l} style={{ flex:1 }}>
                ...
                <div style={{ fontSize:'0.58rem', color: i===step ? 'var(--cyan)' : 'var(--text-muted)', marginTop:3, fontFamily:'var(--font-mono)', textAlign:'center', letterSpacing:'0.05em' }}>{l}</div>
              </div>
            ))}
          </div>
          ...
          {step === 1 && totp && <>
            ...
            <input className="cyber-input" placeholder="000000" maxLength={6} value={totpToken} onChange={e=>setTotpToken(e.target.value.replace(/\D/g,''))} style={{ ...inputStyle, letterSpacing:'0.3em', textAlign:'center', fontSize:'1.15rem' }} />
            <p style={{ fontSize:'0.72rem', color:'var(--amber)', marginBottom:10 }}>⚠ Only Google Authenticator / compatible TOTP apps. No SMS.</p>
          </>}

          {step === 2 && <>
            <p style={{ fontSize:'0.85rem', color:'var(--text-secondary)', marginBottom:14 }}>Save these backup codes securely — each can only be used once:</p>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:7, marginBottom:14 }}>
              {codes.map((c,i) => <div key={i} style={{ fontFamily:'var(--font-mono)', fontSize:'0.78rem', color:'var(--green)', background:'var(--green-glow)', border:'1px solid rgba(0,255,136,.2)', borderRadius:6, padding:'5px 9px', textAlign:'center', letterSpacing:'0.1em' }}>{c}</div>)}
            </div>
          ...
          {step === 3 && newId && <>
            <div style={{ textAlign:'center', marginBottom:20 }}>
              ...
              <h3 style={{ fontFamily:'var(--font-display)', color:'var(--green)', letterSpacing:'0.1em', marginBottom:8 }}>IDENTITY CREATED</h3>
        ...
        {mode === 'login' && <>
          {storedId ? (
            <div style={{ marginBottom:16, padding:'12px 14px', background:'rgba(0,229,255,.05)', border:'1px solid var(--cyan-border)', borderRadius:8 }}>
              <div style={{ fontSize:'0.68rem', color:'var(--text-muted)', fontFamily:'var(--font-mono)', marginBottom:4 }}>STORED IDENTITY</div>
              <div style={{ fontSize:'0.82rem', color:'var(--text-primary)', marginBottom:2 }}>{storedId.alias}</div>
              <div style={{ fontSize:'0.68rem', color:'var(--cyan)', fontFamily:'var(--font-mono)' }}>{truncateDID(storedId.did, 18)}</div>
            </div>
          ) : (
          ...
          <input className="cyber-input" placeholder="000000" maxLength={6} value={loginOTP} onChange={e=>setLoginOTP(e.target.value.replace(/\D/g,''))} style={{ ...inputStyle, letterSpacing:'0.3em', textAlign:'center', fontSize:'1.1rem' }} />
          <p style={{ fontSize:'0.72rem', color:'var(--text-muted)', marginBottom:16 }}>Open Google Authenticator → enter the 6-digit code for Q-IDChain</p>
          <button className="cyber-btn primary" style={{ width:'100%', justifyContent:'center' }} onClick={doLogin}>◉ CONNECT IDENTITY</button>
        </>}

        <p style={{ textAlign:'center', marginTop:18, fontSize:'0.72rem', color:'var(--text-muted)' }}>Your private keys never leave your device.</p>
      </div>
    </div>
  )
}

const labelStyle = { display:'block', fontSize:'0.68rem', color:'var(--text-muted)', fontFamily:'var(--font-mono)', letterSpacing:'0.08em', marginBottom:5, marginTop:10 }
